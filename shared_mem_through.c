...
int main(int argc, char *argv[])
{
	...
	bool result ;
	int   i , j  ;     //pipe fd is for sending the data from parent to child and pipefd1 is for recieving the data from parent to child
	...
	sem_init(&buf->empty, 1, iterations);
	...
	if (pid < 0) {
		...
		/* configure the size of the shared memory segment */
		ftruncate(shm_fd, TOTAL_BUFFER*iterations);

		/* map the shared memory segment to the address space of the process */
		shm_base = mmap(0, TOTAL_BUFFER*iterations, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
		...
		for ( i = 0 ; i < NUM_INPUT ; i++) { 
			...
			for ( j = 0 ; j < iterations ; j++) {   
				...
				ret = consumer(ptr, send_buf, a[i]);
				ptr+=a[i];
			}
			ret = producer2(shm_base, message, length);
		}

		/* remove the mapped memory segment from the address space of the process */
		if (munmap(shm_base, TOTAL_BUFFER*iterations) == -1) {
		...
		for ( i = 0 ; i < NUM_INPUT ; i++) { 
			...
			for ( j = 0 ; j < iterations ; j++) {   
				...
				ret = producer(ptr, data, a[i]);
				ptr+=a[i];
			}
			ret = consumer2(shm_base, send_buf, 3);
